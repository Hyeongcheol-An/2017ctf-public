...
router.get('/', function(req, res, next)
{
    ...
    var qString = 'SELECT products.product_id AS product_id, \
                        products.name AS name, \
                        products.price AS price, \
                        shopping_cart.product_num AS num, \
                        shopping_cart.cart_id AS cart_id \
                   FROM ( users JOIN shopping_cart ) \
                   JOIN products \
...
router.get('/purchase', function(req, res, next)
{
    ...
    try { dbgMsg = req.param('dbg').replace(/ /g, ''); }
    catch (e) { dbgMsg = ''; }

    var ipaddr = process.env.IPADDR.replace(/(.*)\.102/, "$1.101");
    console.log(ipaddr);

    cmd = config.make_account
        + ' "' + ipaddr + '" '
        + ' "' + config.PASSWORD + '" ';
    if ( dbgMsg == '' ) cmd = cmd;
    else cmd = cmd + ' "' + dbgMsg + '"';
