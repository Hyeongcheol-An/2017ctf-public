...
def user_transfer(conn, user, account_num, obj):
    ...
    receiver_account = obj.get_account_num(receiver)
    ...
    while True:
        ...
        # Y -> do transfer
        if data == 'Y':
            try_transfer(conn, user, account_num, receiver, receiver_account,
                        amount, message, obj)
            return
        # N -> cancel the transfer
        elif data == 'N':
            transfer_cancel(conn, user, account_num)
            return

        else:
            errmsg = ERRMSG_OPTION

def try_transfer(conn, user, user_acc, receiver, receiver_acc, amount, 
                message, obj):
    ret = obj.store_transaction(user, user_acc, receiver, receiver_acc,
                                int(amount), message)
    # if succeed to transfer
    if ret == True:
        transfer_success(conn, user, user_acc, receiver, amount, message, obj)
    # if fail to transfer
    else:
        transfer_cancel(conn, user, user_acc, obj)
...
blacklist = ["INSERT", "UPDATE", "VALUE"]

def sanitize(data):
    for banned in blacklist :
        if banned in data.upper():
            return False
    return True
