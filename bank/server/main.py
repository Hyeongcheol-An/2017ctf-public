import socket, sys, re, base64, os, inspect, traceback
...
def server(input_passphrase, input_mysql_pw, service_ip):
    thread_list = []

    utils.mysql_pw = input_mysql_pw
    # Create a TCP/IP socket
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

    # Bind the socket to the port
    server_address = (service_ip, 1588)
